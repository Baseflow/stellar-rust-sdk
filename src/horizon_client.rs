...
impl HorizonClient {
    ...
    /// # Returns
    ///
    /// On successful execution, returns a `Result` containing an [`EffectsForTransactionResponse`], which includes
    /// the list of effects obtained from the Horizon server. If the request fails, it returns an error within `Result`.
    ///
    /// # Example
    ...
    ) -> Result<EffectsForTransactionResponse, String> {
        self.get::<EffectsForTransactionResponse>(request).await
    }
